...
function K2Module({ onDone }) {
  ...
  const evaluate = () => {
    ...
    setResult(depth > 100 ? "green" : depth > 50 ? "orange" : "red");
  };
  ...
  if (result) {
    ...
    return (
      <div style={{ textAlign: "center", padding: "32px 0" }}>
        ...
        <div style={{ color: r.color, fontSize: 24, fontWeight: 800, marginBottom: 10 }}>{r.label}</div>
        <div style={{ color: C.gray, fontSize: 14, maxWidth: 460, margin: "0 auto 28px", lineHeight: 1.7 }}>{r.text}</div>
        <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
          <button onClick={() => { setResult(null); setStep(0); setVals(Array(5).fill("")); }}
            style={{ padding: "10px 22px", borderRadius: 8, background: "transparent", border: `1px solid ${C.border}`, color: C.gray, cursor: "pointer", fontSize: 14 }}>
            Neuer Test
          </button>
          <button onClick={onDone}
            style={{ padding: "10px 22px", borderRadius: 8, background: C.teal, border: "none", color: C.white, cursor: "pointer", fontSize: 14, fontWeight: 600 }}>
            Canvas Assessment →
          </button>
        </div>
      </div>
